...
\]
\end{enumerate}
\begin{sg}[Der Kräftefreie Kreisel]
\fixme[Grafik fehlt]
\begin{enumerate}[a)]
...
\[
\hat I= \begin{pmatrix} I_{||} & & 0 \\ & I_{\perp x} & \\ 0 & & I_{xy} \end{pmatrix} %Tensor
\]
...
\end{seg}
\end{sg}
\begin{sg}[Wie sieht eine Nutationsbewegung aus?]
\fixme[Grafik fehlt]
 \begin{itemize}
...
\[
% \cyrdje \cyrdje= \frac{h}{2\pi}= 1.0544 \cdot 10^{-34} \frac{\kg \m^2}{\s} %zeichen nachschauen \cyrdje
\]
...
\begin{ex}[Rotation eines Stickstoffmoleküls]
 $\approx$ starre Hantel.\\
\fixme[zwei Slides fehlen]
\end{ex}
